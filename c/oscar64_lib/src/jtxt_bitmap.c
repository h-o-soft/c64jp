...
void jtxt_bcls(void) {
  ...
  uint16_t bitmap_addr =
      JTXT_BITMAP_BASE + (uint16_t)jtxt_state.bitmap_top_row * 320;
  uint16_t screen_addr =
      JTXT_BITMAP_SCREEN_RAM + (uint16_t)jtxt_state.bitmap_top_row * 40;

  for (uint8_t row = jtxt_state.bitmap_top_row;
       row <= jtxt_state.bitmap_bottom_row; row++) {
...
void jtxt_bscroll_up(void) {
  ...
  // Scroll each row
  for (uint8_t i = jtxt_state.bitmap_top_row; i < jtxt_state.bitmap_bottom_row;
       i++) {
    // Copy bitmap data
...
void jtxt_draw_font_to_bitmap(uint16_t char_code) {
  ...
  // Set color information
  uint16_t color_addr = JTXT_BITMAP_SCREEN_RAM +
                        (uint16_t)jtxt_state.cursor_y * 40 +
                        jtxt_state.cursor_x;
  POKE(color_addr, jtxt_state.bitmap_color);
