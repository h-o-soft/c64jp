...
const struct bindings* current_bindings;
...
void qe_itoa(uint16_t val, char* buf)
{
    uint8_t i = 0;
    do
    {
...
// Simple strtok implementation for Oscar64
...
static bool insert_file(const char* path)
{
    ...
    // Open file for reading using Oscar64 kernalio
    krnio_setnam(path);
    if (!krnio_open(1, 8, 0))
    {
        print_status("Open failed");
        return false;
    }

    // Read file contents into gap
    uint8_t* write_ptr = gap_start;
    int bytes_read;
    char read_buffer[128];

    while ((bytes_read = krnio_read(1, read_buffer, sizeof(read_buffer))) > 0)
    {
        // Convert CR to LF while copying
    ...
    // Update gap_start
    uint16_t bytes_loaded = write_ptr - gap_start;
    gap_start = write_ptr;

    if (bytes_loaded > 0)
        dirty = true;

    format_status_with_path("Read ", path);
    return true;
#else
    (void)path;
...
bool save_file(void)
{
    ...
    // Open file for writing using Oscar64 kernalio
    krnio_setnam(current_filename);
    if (!krnio_open(1, 8, 1))
    {
        print_status("Open failed");
        return false;
    ...
    // Write data from buffer_start to gap_start
    int written1 = krnio_write(1, (char*)buffer_start, gap_start - buffer_start);

    // Write data from gap_end to buffer_end
    int written2 = krnio_write(1, (char*)gap_end, buffer_end - gap_end);

    krnio_close(1);

    bool ok = (written1 >= 0 && written2 >= 0);
    if (!ok)
    {
...
void quit(void)
{
    ...
    __asm {
        jsr $FF8A  // RESTOR: Restore vectors
...
{
    current_bindings = &delete_bindings;
    command_count = count;
}

void enter_zed_mode(uint16_t count)
{
    current_bindings = &zed_bindings;
    command_count = count;
}

void enter_change_mode(uint16_t count)
{
    current_bindings = &change_bindings;
    command_count = count;
}
...
int main(void)
{
    ...
    screen_setcursor(0, 0);
    render_screen(first_line);
    current_bindings = &normal_bindings;
    ...
    for (;;)
    {
        ...
        const char* cmdp = strchr(current_bindings->keys, c);
        if (cmdp)
        {
            command_t* cmd = current_bindings->callbacks[cmdp - current_bindings->keys];
            uint16_t count = command_count;
            if (count == 0)
            {
                // Oscar64: Simplified - removed function pointer comparison
                // Original code checked if cmd == goto_line for UINT16_MAX
                count = 1;
            }
            command_count = 0;
            ...
            set_status_line("");
            cmd(count);
            if (current_bindings->name)
                set_status_line(current_bindings->name);
        }
        else
        {
